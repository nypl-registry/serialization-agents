...
module.exports = function shadowcatSerializeViafAgents (cb) {
  ...
  if (cluster.isMaster) {
      ...
      botSplit.forEach((x) => {
        ...
        worker.on('message', function (msg) {
          ...
          if (msg.counter) {
            ...
            process.stdout.write(clc.black.bgYellowBright(`Shadowcat VIAF Agents Serialization: ${Math.floor(totalDone / totalTodo * 100)}% ${++totalDone} Bots:${totalBots}`))
          }
        })
      })
    })
    ...
    setInterval(() => {
      if (!worked) console.log(`${cluster.worker.id} has not worked in the last few min:`, workedLastOn)
      worked = false
    }, 120000)
