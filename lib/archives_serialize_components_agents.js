...
module.exports = function (cb) {
  ...
  if (cluster.isMaster) {
    ...
    crier.registrySay(':vertical_traffic_light: Agent Serialization Archives Components starting.')
    ...
    var spawnWorker = () => {
      ...
      worker.on('message', function (msg) {
        ...
        if (msg.counter) {
          ...
          activeMessage = `Archives Components Agent Serialization: ${percent}% ${totalDone} Components worked. Agents Added: ${totalAgentsAdded} Bots:${totalBots}`
    ...
    cluster.on('exit', (worker, code, signal) => {
      ...
      setTimeout(() => {
        if (Object.keys(cluster.workers).length === 0) {
          crier.registrySay(activeMessage)
          crier.registrySay(':checkered_flag: Archives Components Agent Serialization: Complete')
          if (cb) {
            setTimeout(() => {
              cb()
        ...
        db.returnCollectionRegistry('archivesComponents', (err, archivesComponents) => {
          ...
          _(archivesComponents.find({$and: [{mssDb: {$gte: msg.work.start}}, {mssDb: {$lt: msg.work.end}}]}, {'agents': 1, 'mssDb': 1}))
            .map((archivesComponents) => {
              if (++localCounter % 1000 === 0 && localCounter !== 0) process.send({ counter: 1000 })
              // for debuging
              worked = true
