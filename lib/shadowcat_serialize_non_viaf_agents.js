...
module.exports = function (cb) {
  ...
  if (cluster.isMaster) {
    ...
    crier.registrySay(':vertical_traffic_light: Agent Serialization Shadowcat agents without VIAF starting.')
    ...
    var spawnWorker = () => {
      ...
      worker.on('message', function (msg) {
        ...
        if (msg.counter) {
          ...
          activeMessage = `Shadowcat Non-VIAF Agent Serialization: ${percent}% ${totalDone} Bibs worked. Agents Added: ${totalAgentsAdded} Bots:${totalBots}`
    ...
    cluster.on('exit', (worker, code, signal) => {
      ...
      setTimeout(() => {
        if (Object.keys(cluster.workers).length === 0) {
          crier.registrySay(activeMessage)
          crier.registrySay(':checkered_flag: Shadowcat without VIAF Agent Serialization: Complete')
          if (cb) {
            setTimeout(() => {
              cb()
      ...
      if (msg.work) {
        ...
        db.returnCollectionShadowcat('bib', (err, bibs) => {
          ...
          _(bibs.find({$and: [{_id: {$gte: msg.work.start}}, {_id: {$lt: msg.work.end}}]}, {'sc:agents': 1, 'sc:research': 1}))
            ...
            .compact()
            .map(_.curry(shadowcatUtils.lookupShadowcatAgentsInRegistryAgentsByName)) // fill in any existing agents
            .nfcall([])
            .series()
            .map((bib) => {
              var updateAgents = []
              ...
              bib['sc:agents'].forEach((agent) => {
                ...
                // we have viaf as unique in agents, so make a viaf id for it
                if (!r.viaf) r.viaf = 'noViaf' + Date.now() + (Math.floor(Math.random() * (1000000 - 1)) + 1) + (Math.floor(Math.random() * (10000 - 1)) + 1)
            ...
            .map(_.curry(shadowcatUtils.updateShadowcatAgentsByName)) // add/update the data to the agents collection
            .nfcall([])
            .series()
            .done((bib) => {
              console.log('Done Working. #', cluster.worker.id)
