...
module.exports = function (cb) {
  ...
  if (cluster.isMaster) {
    ...
    crier.registrySay(':vertical_traffic_light: Agent Serialization - Starting Enumerate Agents.')
    ...
    var spawnWorker = () => {
      ...
      worker.on('message', function (msg) {
        ...
        if (msg.counter) {
          ...
          activeMessage = `Enumerate Agents: ${percent}% ${totalDone} Components worked. Bots:${totalBots}`
    ...
    var updateRegistryId = (agent, callback) => {
      db.returnCollectionRegistry('agents', (err, agents) => {
        if (err) console.log(err)
        agents.update({registry: agent.registry}, { $set: {registry: agent.useId} }, (err, result) => {
          if (err) {
