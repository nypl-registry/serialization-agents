    ...
    var spawnWorker = () => {
      ...
      worker.on('message', function (msg) {
        ...
        if (msg.counter) {
          ...
          activeMessage = `TMS Agent Serialization: ${percent}% ${totalDone} Bibs worked. Agents Added: ${totalAgentsAdded} Bots:${totalBots}`
    ...
    cluster.on('exit', (worker, code, signal) => {
      ...
      setTimeout(() => {
        if (Object.keys(cluster.workers).length === 0) {
          crier.registrySay(activeMessage)
          crier.registrySay(':checkered_flag: TMS Agent Serialization: Complete')
          if (cb) {
            setTimeout(() => {
              cb()
