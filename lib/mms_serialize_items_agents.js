...
module.exports = function (cb) {
  ...
  if (cluster.isMaster) {
    ...
    crier.registrySay(':vertical_traffic_light: Agent Serialization MMS Items starting.')
    ...
    var spawnWorker = () => {
      ...
      worker.on('message', function (msg) {
        ...
        if (msg.counter) {
          ...
          activeMessage = `MMS Items Agent Serialization: ${percent}% ${totalDone} Items worked. Agents Added: ${totalAgentsAdded} Bots:${totalBots}`
    ...
    cluster.on('exit', (worker, code, signal) => {
      ...
      setTimeout(() => {
        if (Object.keys(cluster.workers).length === 0) {
          crier.registrySay(activeMessage)
          crier.registrySay(':checkered_flag: MMS Items Agent Serialization: Complete')
          if (cb) {
            setTimeout(() => {
              cb()
    ...
    process.on('message', (msg) => {
      ...
      if (msg.work) {
        ...
        db.returnCollectionRegistry('mmsItems', (err, mmsItems) => {
          ...
          _(mmsItems.find({$and: [{mmsDb: {$gte: msg.work.start}}, {mmsDb: {$lt: msg.work.end}}]}, {'agents': 1, 'mmsDb': 1}))
            .map((mmsItem) => {
              ...
              mmsItem.subjects.forEach((subject) => {
              ...
              if (!mmsItem.agents) return ''
              if (mmsItem.agents.length === 0) return ''

              return mmsItem
            })
            .compact()
            .map(_.curry(mmsUtils.lookupAgentsInViaf)) // fill in any existing viaf data
            .nfcall([])
            .series()
            .map(_.curry(mmsUtils.lookupAgentsInViafByNaf)) // fill in any existing viaf data
            .nfcall([])
            .series()
            ...
            .map((mmsItem) => {
              ...
              mmsItem['agents'].forEach((agent) => {
                if (!agent.agent && agent.namePart) {
                  agent.source = {source: 'mmsItems', id: mmsItem.mmsDb}
